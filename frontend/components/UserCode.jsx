...
function UserCode({updateResultsData}) {
  ...
  async function postData() {
    ...
    try {
      ...
      const response = await axios.post(code_submit_url, data)
  ...
  return (
    <div className="flex-col justify-center w-full">
      ...
      />
      {/* 코드 실행 버튼 + 런타임 출력 부분 */}
      <div className="flex justify-end border-black-1 space-between space-x-2 p-2 items-center">
        {/* <Runntime /> */}
        <span className={codeRuntimeClassName}>{codeRuntime}</span>
        <button className="bg-primary-green border-black rounded text-white p-2 " id="compile-btn" onClick={getEditorValue}>
          Run
        </button> 
      </div>
